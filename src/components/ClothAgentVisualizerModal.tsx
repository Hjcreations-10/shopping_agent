...
export const ClothAgentVisualizerModal: React.FC<ClothAgentVisualizerModalProps> = ({
  ...
  const [activeViewMode, setActiveViewMode] = useState<'scan' | 'fabric' | 'outfit'>('scan');

  const isClothing = product?.category === 'clothing';
  const clothingItem = isClothing ? (product as ClothingProduct) : null;

  // Find complementary item from catalog for outfit synergy
  useEffect(() => {
    if (!product) return;
    setIsScanning(true);
  ...
  // Custom analysis pins based on product
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 overflow-y-auto bg-stone-900/60 backdrop-blur-xs">
        ...
        >
            ...
            <div className="flex items-center space-x-2.5">
              ...
              <div>
                ...
                <p className="text-xs text-stone-500">
                  Real-time fabric inspection, color harmony, and fit suitability analysis
                </p>
              </div>
            </div>
          ...
          <div className="p-6 grid grid-cols-1 lg:grid-cols-12 gap-6">
            {/* Left: Interactive Cloth Visualizer Canvas with Agent Scan Laser */}
            <div className="lg:col-span-6 flex flex-col">
              <div className="relative rounded-2xl overflow-hidden bg-stone-900 border border-stone-800 aspect-square flex items-center justify-center group shadow-inner">
                ...
                <img
                  ...
                  alt={product.name}
                  className="w-full h-full object-cover object-center filter brightness-95 contrast-105 transition-transform duration-500 group-hover:scale-105"
                  referrerPolicy="no-referrer"
                />

                {/* Animated Holographic Agent Laser Scan Line */}
                {isScanning && (
                  <motion.div
                    className="absolute inset-x-0 h-1 bg-gradient-to-r from-transparent via-emerald-400 to-transparent shadow-[0_0_15px_#10b981] z-20 pointer-events-none"
                    animate={{
                      top: ['5%', '90%', '5%']
                    }}
                    transition={{
                ...
                {/* Status HUD Overlay in top corner */}
                ...
                {/* Rescan Button in bottom corner */}
                <button
                  onClick={() => {
                    setIsScanning(true);
                ...
                >
                  ...
                  <span>Re-Scan Fabric</span>
                </button>

                {/* Interactive Hotspot Pins on the Garment */}
                {pins.map((pin) => {
            ...
            {/* Right: Agent Reasoning, Fabric Telemetry, and Outfit Coordination */}
            <div className="lg:col-span-6 flex flex-col justify-between space-y-4">
              <div>
                ...
                {/* TAB 1: FABRIC SCAN & TAILORING TELEMETRY */}
                {activeViewMode === 'scan' && (
                  <div className="mt-4 space-y-3">
                    ...
                    {/* Color Harmony Palette (for clothing) */}
                      ...
                      <div className="p-3 rounded-xl bg-stone-50 border border-stone-200">
                        ...
                        <div className="flex items-center gap-2 text-[10px] text-stone-600">
                          ...
                          <div className="flex items-center gap-1">
                            <span className="w-3.5 h-3.5 rounded-full bg-slate-700 border border-stone-300" title="Charcoal" />
                            <span className="w-3.5 h-3.5 rounded-full bg-stone-300 border border-stone-300" title="Khaki" />
                            <span className="w-3.5 h-3.5 rounded-full bg-blue-900 border border-stone-300" title="Navy" />
                            <span className="w-3.5 h-3.5 rounded-full bg-amber-100 border border-stone-300" title="Beige" />
                          </div>
                        </div>
                      </div>
                    )}

                    {/* Agent Stylist Decision Summary */}
                    <div className="p-3 rounded-xl bg-stone-900 text-stone-200 border border-stone-800 text-xs space-y-1.5">
                      ...
                      <p className="text-[11px] leading-relaxed text-stone-300">
                        &quot;Selected specifically for its high yarn count that resists wrinkling during presentations. The subtle {clothingItem?.color || 'finish'} maintains academic poise without overspending.&quot;
                      </p>
                    </div>
                  </div>
                )}

                {/* TAB 2: VIRTUAL OUTFIT COORDINATOR */}
                {activeViewMode === 'outfit' && selectedCompanion && (
                  <div className="mt-4 space-y-3">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          <h5 className="font-bold text-xs text-indigo-950">
                            Coordinated Outfit Compatibility
                          </h5>
                    ...
                    <div className="grid grid-cols-2 gap-3 items-center">
                      <div className="p-2.5 rounded-xl border border-stone-200 bg-white flex items-center gap-2">
                        <img
                          src={product.imageUrl}
                          alt={product.name}
                        ...
                        <div className="overflow-hidden">
                          <span className="text-[9px] font-bold uppercase text-stone-400 block truncate">
                            Top Piece
                          </span>
                      ...
                      <div className="p-2.5 rounded-xl border border-indigo-200 bg-indigo-50/30 flex items-center gap-2">
                        <img
                        ...
                        />
                        <div className="overflow-hidden">
                          <span className="text-[9px] font-bold uppercase text-indigo-500 block truncate">
                            Coordinating Bottom
                          </span>
                          <p className="text-xs font-bold text-stone-900 truncate">
